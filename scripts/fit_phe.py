...
def fit_protons(initial_params):
    ...
    m = Minuit(chi2_function, xi=initial_params[0], alpha=initial_params[1], 
               lnEb=initial_params[2], dalpha=initial_params[3])
...
def fit_helium(initial_params):
    ...
    """Perform the chi-squared fit for the Helium model."""
    # Perform minimization
...
if __name__ == "__main__":
    ...
    initial_guess = [2., 2.8, np.log(500.), 0.20]
    fit_protons(initial_guess)
    initial_guess = [0.2, 2.7, np.log(250.), 0.20]
